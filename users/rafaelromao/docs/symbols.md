...
- Number are available in the right side of the Lower layer, also requiring two hands, but that can be mitigated using a combo to toggle the layer on using only the rigth hand.
...
- A combo with the three fingers in the home row can be used to toggle these layers off.
